...
class Emeasure(object):
    ...
    def cal_em_with_threshold(self, pred: np.ndarray, gt: np.ndarray, threshold: float) -> float:
        ...
        if self.gt_fg_numel == 0:
            ...
            align_FM = pred - mu_FM
            align_GT = gt - mu_GT
            align_matrix_value =2 * (align_GT * align_FM) / (align_GT * align_GT + align_FM * align_FM + _EPS)
            enhanced_matrix = ((align_matrix_value + 1) ** 2) / 4

        em = enhanced_matrix.sum() / (self.gt_size - 1 + _EPS)
        return em

    def get_results(self) -> dict:
        adaptive_em = np.mean(np.array(self.adaptive_ems, dtype=_TYPE))
        return dict(em=dict(adp=adaptive_em))
