...
class Con3x3WithBnRelu(nn.Module):
    def               __init__(self,in_ch,out_ch):
        ...
        self.con3x3 = nn.Conv2d(in_ch, out_ch,
...
class MyModel(nn.Module):
    def __init__(self):
        ...
        self.s2_final_conv1 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.s2_final_conv2 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.s2_final_conv3 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
        self.s2_final_conv4 = nn.Conv2d(64, 1, kernel_size=3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        result1 = self.s2_final_conv1(up1)
        result2 = self.s2_final_conv2(up2)
        result3 = self.s2_final_conv3(up3)
        result4 = self.s2_final_conv4(up4)

        result =  self.s2_final_conv(torch.cat((up1,up2,up3,up4),dim=1))

        return torch.sigmoid(result),\
               torch.sigmoid(result4),\
               torch.sigmoid(result3),\
               torch.sigmoid(result2),\
               torch.sigmoid(result1)

if __name__ == '__main__':


    # summary(MyModel(),(3,29,256,256),device='cpu')
    #
    for i in  MyModel().named_parameters():
        print(i[0])